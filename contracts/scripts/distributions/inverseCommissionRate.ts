...
async function main() {
  try {
    ...
    // Inverse Commission Rate Distribution
    // Compute total inverse commission rate
    let totalInverseCommissionRate = collators.reduce(
      (total, validator) =>
        total + 1 / parseFloat(validator.commission.commission_rates.rate),
      0
    );

    // Compute amounts to stake based on inverse commission rate
    let amounts = collators.map((validator) =>
      ethers.utils
        ...
        .mul(
          ethers.utils
            .parseEther("1")
            .div(parseFloat(validator.commission.commission_rates.rate))
        )
        .div(totalInverseCommissionRate)
    );
